...
start_robot_io() {
  ...
  if [[ "$ROBOT_VIDEO_ENABLE" == "1" || "$ROBOT_VIDEO_ENABLE" == "true" ]]; then
    ...
    if [[ "$ROBOT_USB_CAMERA_RTSP_ENABLE" == "1" || "$ROBOT_USB_CAMERA_RTSP_ENABLE" == "true" ]]; then
      echo "[ssh] usb cams    : selected=${ROBOT_USB_CAMERA_RTSP_CAMERAS} base=${ROBOT_BASE_CAMERA_DEVICE}->/${ROBOT_BASE_CAMERA_PATH}, left=${ROBOT_WRIST_LEFT_CAMERA_DEVICE}->/${ROBOT_WRIST_LEFT_CAMERA_PATH}, right=${ROBOT_WRIST_RIGHT_CAMERA_DEVICE}->/${ROBOT_WRIST_RIGHT_CAMERA_PATH}"
    fi
  else
